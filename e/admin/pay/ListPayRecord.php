...
$query="select id,userid,username,orderid,money,posttime,paybz,type,payip,status from {$dbtbpre}enewspayrecord";
...
<form name="form2" method="post" action="ListPayRecord.php" onsubmit="return confirm('确认要删除?');">
  ...
  <?=$ecms_hashur['form']?>
    <tr class="header"> 
      ...
      <td width="8%" height="25"><div align="center">状态</div></td>
    </tr>
    <?
    ...
    <tr bgcolor="#FFFFFF" onmouseout="this.style.backgroundColor='#ffffff'" onmouseover="this.style.backgroundColor='#C3EFFF'"> 
      ...
      <td height="25"><div align="center"><?=$r[type]?></div></td>
        <td height="25" style="text-align: center;">
            <?php
                if($r[status] == 1)
                    echo "<span style='background-color:#d2f0cd;color:#22401e;padding:2px 16px 4px;border-radius: 20px;'>已支付</span>";
                else
                    echo "<span style='background-color:#f9ecc7;color:#6f4028;padding:2px 16px 4px;border-radius: 20px;'>未支付</span>";
            ?>
        </td>
    </tr>
    <?
